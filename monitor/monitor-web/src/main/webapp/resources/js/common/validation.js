function validator(options) {
	...
	}, this.allowedElement = [ 'input', 'INPUT', 'select', 'SELECT' ], this.isAllowed = function(input) {
	...
	this.validationForm = function(input, callback) {
		...
		if (!this.isAllowed(input)) {
			console.log('u r kid me,  u r sure to validation a dom element without value');
		}
		...
		var defultValue = $(input).data("default") ? $(input).data("default") : this.rules[type].defaultValue;// 默认值
		var errorMsg = $(input).data("error") ? $(input).data("error") : this.rules[type].errorMsg;// 错误信息
